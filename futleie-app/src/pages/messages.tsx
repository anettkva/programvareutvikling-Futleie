...
import { fetchMessagedUsers, fetchMessagesBetweenUsers, formatMessages, fetchUsers, sendMessage } from "@/services/messages";
...
const Messages: React.FC = () => {
  ...
  const handleStartConversationFromAd = async (userId: number, receiverId: number, itemId: string) => {
    try {
      console.log(`Starting conversation from ad: user ${userId} to receiver ${receiverId} about item ${itemId}`);
      ...
      let existingMessages = await fetchMessagesBetweenUsers(userId, receiverId);
      ...
      if (existingMessages.length === 0) {
        ...
        existingMessages = await fetchMessagesBetweenUsers(userId, receiverId);
      }
      ...
      setUserChats(prevChats => {
        ...
        const existingChatIndex = updatedChats.findIndex(chat => chat.id === receiverId);
  ...
  useEffect(() => {
    const startConversation = async () => {
      if (receiverId && itemId && currentUserId && !messageSent) {
        await handleStartConversationFromAd(currentUserId, parseInt(receiverId), itemId);
        setMessageSent(true);
      }
    };
  ...
  const loadUserData = async () => {
    try {
      ...
      for (const user of messagedUsers) {
        const messages = await fetchMessagesBetweenUsers(userId, user.id);
  ...
  const handleNewChat = (e: React.FormEvent) => {
    ...
    const existingChat = userChats.find(chat => chat.id === newChatUser.id);
    ...
    if (existingChat) {
      ...
      setUserChats(prev => prev.map(chat => ({ ...chat, isActive: false })).concat(newUserChat));
    }
        ...
        <div className="flex-1">
          {userChats.length > 0 ? (
            ...
            <div className="flex items-center justify-center h-full">
              <p className="text-gray-500">Du har ingen meldinger ennå. Start en ny samtale!</p>
            </div>
          )}
        </div>
      </div>
