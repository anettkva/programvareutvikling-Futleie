...
const HistoryItem: React.FC<{
  ...
  /**
   * Håndterer innsending av vurdering
   * @returns
   */
  const handleRatingSubmit = async () => {
    try {
      ...
      console.log(`Submitting rating ${item.rating} for rental ID ${item.id}`);
  ...
  return (
    <Card className="mb-4 w-full h-64 flex flex-col overflow-hidden border-[#FEDEC7]">
      <CardHeader className="flex-shrink-0 pb-2">
        ...
        <CardDescription className="truncate">{item.username}</CardDescription>
      </CardHeader>
        ...
        {[...Array(5)].map((_, i) => {
          ...
          return (
            <Star
              ...
              onClick={() =>
                !isRated && onRatingChange(itemId, type, starValue)
              }
