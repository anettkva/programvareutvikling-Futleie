...
export const AppSidebar: React.FC = () => {
  ...
  /**
   * Logger ut bruker og fjerner cookie
   */
  const handleLogout = () => {
    Cookie.remove("user");
    navigate("/login");
  ...
  /**
   * Sjekk om bruker er admin og legg til admin-menyvalg
   */
  useEffect(() => {
    async function checkIsAdmin() {
      ...
      if (userId) {
        ...
        if (error) {
          ...
          if (data.admin && !items.some((item) => item.title === "Admin")) {
            setItems([
              ...items,
              {
                title: "Admin",
                ...
                >
                  <img
                    ...
                    className={`object-contain ${
                      state === "collapsed" ? "w-8 h-auto -ml-1" : "w-28 h-auto"
                    }`}
                  />
                </a>
              </SidebarMenuItem>
