...
/**
 * Form for å endre annonse
 * @returns Form for å endre annonse
 */
const ChangeItemForm: React.FC = () => {
  ...
  /**
   * Henter annonse og bilder fra Supabase
   */
  useEffect(() => {
    async function fetchItem() {
      const { data, error } = await supabaseClient
        .from("Items")
        .select("title, description, category, lat, lng")
        .eq("id", itemId)
        .single();
      ...
      if (data) {
        ...
        // Set the userLocation state if coordinates exist
        if (data.lat && data.lng) {
          setUserLocation({
            lat: data.lat,
            lng: data.lng,
          });
          setLocationStatus("Posisjon lastet");
  ...
  /**
   * Håndterer opplasting av bilder
   * @param e
   */
  ...
  return (
    <>
      ...
      <div className="mt-4">
        <h2 className="text-lg font-semibold mb-2">Eksisterende bilder</h2>
        {images.length === 0 ? (
          <p>Ingen bilder å vise</p>
        ) : (
          images.map((img) => (
            <div key={img.id} className="flex items-center gap-2 mt-2">
              <img
                src={img.image_url}
                alt="Item"
