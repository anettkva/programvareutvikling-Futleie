...
export function Chat({
    ...
    React.useEffect(() => {
        ...
        // Set up subscription to the messages table
        try {
            console.log("Creating channel subscription...");
            const subscription = supabaseClient
                .channel("messages-channel-" + currentUserId) // More unique channel name
                .on(
                    "postgres_changes",
                    {
                        event: "INSERT",
                    ...
                    async (payload) => {
                        ...
                        // Skip processing if we were the sender (avoid duplicates)
                        // Note: Removing is_processed_locally check for now since it might not exist
                        if (message.sender_id === currentUserId) {
                            console.log(
                                "Skipping our own sent message to avoid duplication"
                            );
                            return;
                        }
                        ...
                        // Get other party details if needed
                        let otherPartyName = "";
                        ...
                        {activeChat.messages.length > 0 ? (
                            ...
                            <div className="flex items-center justify-center h-full">
                                <p className="text-gray-500">
                                    Ingen meldinger ennå. Send en melding for å
                                    starte samtalen!
                                </p>
                            </div>
                        )}
